...
export class CartAutomation {
  ...
  async addToCart(): Promise<CartOperation> {
    ...
    try {
      ...
      const loginButton = await page.$(
        'button[type="submit"], .login-button, [class*="login"]'
      );
      if (loginButton) {
      ...
      if (!isLoggedIn) {
        return {
          success: false,
          message:
            "Failed to login to Amul account. Please check your credentials.",
        };
      }
      ...
      const isInCart = await page.evaluate(() => {
        const cartIndicator = document.querySelector(
          '[class*="cart"], [class*="basket"], .cart-count'
        );
        return (
          cartIndicator &&
          cartIndicator.textContent &&
          parseInt(cartIndicator.textContent) > 0
        );
      });
  ...
  async checkCartStatus(): Promise<CartOperation> {
    ...
    try {
      ...
      // Navigate to cart
      await page.goto("https://shop.amul.com/en/checkout/cart", {
        waitUntil: "networkidle2",
      });
      await new Promise((resolve) => setTimeout(resolve, 2000));
      ...
      const cartInfo = await page.evaluate(() => {
        const cartItems = document.querySelectorAll(
          '[class*="cart-item"], .cart-item, .item'
        );
        const totalElement = document.querySelector(
          '[class*="total"], .cart-total, .total'
        );
